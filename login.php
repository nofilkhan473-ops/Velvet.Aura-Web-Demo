...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL) || !preg_match($email_pattern, $email)) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if ($password === $user['password'] || password_verify($password, $user['password'])) {
                ...
                if (isset($_SESSION['guest_cart']) && !empty($_SESSION['guest_cart'])) {
                    foreach ($_SESSION['guest_cart'] as $guest_item) {
                        ...
                        $check = mysqli_query($conn, "SELECT id, quantity FROM cart WHERE user_id = {$user['id']} AND product_id = $product_id");
                        if (mysqli_num_rows($check) > 0) {
                            $existing = mysqli_fetch_assoc($check);
                            $new_qty = $existing['quantity'] + $quantity;
                            mysqli_query($conn, "UPDATE cart SET quantity = $new_qty WHERE user_id = {$user['id']} AND product_id = $product_id");
                        } else {
                            mysqli_query($conn, "INSERT INTO cart (user_id, product_id, quantity) VALUES ({$user['id']}, $product_id, $quantity)");
                        }
                    }
                ...
                if (isset($_SESSION['guest_wishlist']) && !empty($_SESSION['guest_wishlist'])) {
                    foreach ($_SESSION['guest_wishlist'] as $product_id) {
                        $check = mysqli_query($conn, "SELECT id FROM wishlist WHERE user_id = {$user['id']} AND product_id = $product_id");
                        if (mysqli_num_rows($check) == 0) {
                            mysqli_query($conn, "INSERT INTO wishlist (user_id, product_id) VALUES ({$user['id']}, $product_id)");
                        }
                    }
            ...
            $error = 'No account found with this email address. Please <a href="register.php">register here</a>';
        }
    }
}
?>
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .login-section { min-height: calc(100vh - 200px); display: flex; align-items: center; justify-content: center; padding: 60px 0; position: relative; z-index: 10; }
        .login-card { background: rgba(30, 18, 12, 0.7); backdrop-filter: blur(20px); border-radius: 48px; padding: 50px; max-width: 480px; width: 100%; margin: 0 auto; border: 1px solid rgba(212,165,116,0.3); box-shadow: 0 25px 50px rgba(0,0,0,0.3); transition: transform 0.3s ease; }
        .login-card:hover { transform: translateY(-5px); border-color: rgba(212,165,116,0.5); }
        .login-card h2 { font-size: 42px; font-weight: 800; margin-bottom: 10px; text-align: center; background: linear-gradient(135deg, #F5E6D3, #D4A574); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .login-card p { color: #D4B5A7; margin-bottom: 35px; text-align: center; font-size: 14px; }
        ...
        .btn-login { width: 100%; background: linear-gradient(135deg, #5C2E1A, #8B4513); color: #F5E6D3; padding: 15px; border: none; border-radius: 60px; font-weight: 700; font-size: 14px; cursor: pointer; transition: all 0.3s; letter-spacing: 2px; margin-top: 10px; position: relative; overflow: hidden; }
        ...
        .btn-login:hover { transform: translateY(-3px); box-shadow: 0 10px 30px rgba(92,46,26,0.5); background: linear-gradient(135deg, #8B4513, #5C2E1A); }
        
        .login-footer { text-align: center; margin-top: 25px; font-size: 13px; color: #C4A484; }
        .login-footer a { color: #D4A574; font-weight: 600; text-decoration: none; transition: color 0.3s; }
        .login-footer a:hover { color: #F5E6D3; text-decoration: underline; }
        ...
        @media (max-width: 992px) { 
            ...
            .login-card { padding: 30px; margin: 0 20px; }
        ...
        <div class="login-card" data-aos="fade-up" data-aos-duration="800">
            ...
            <form method="POST" id="loginForm">
                ...
                <div class="form-group">
                    ...
                    <input type="email" name="email" id="loginEmail" placeholder="name@gmail.com" required>
                </div>
                <div class="form-group">
            ...
            <div class="login-footer">Don't have an account? <a href="register.php">Create Account</a></div>
            <div class="login-footer"><a href="forgot-password.php">Forgot Password?</a></div>
        </div>
    </div>
</section>
